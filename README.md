...
1. Compute the normalized mesh Laplacian with
	a. uniform weights
	b. and Cotangent weights.
2. Implement basic mesh smoothing
	a. using the explicit (or forward) Euler method, 
	b. and using the implicit (or backward) Euler method.
	c. Implement exact volume preservation. 
3. Implement least-squares mesh smoothing. 
4. Implement detail preserving triangle shape optimization.
5. Implement spectral smoothing.
	a. Implement a non-normalized version of the uniform and Cotangent Laplacian. 
	b. Implement the spectral smoothing algorithm. 
6. Compute and visualize the discrete mean curvature and the discrete Gaussian curvature. 
7. Discuss the following questions about mesh smoothing:
	a. How does recomputation of the Laplacian after every iteration affect different smoothing methods?
	b. How do explicit and implicit smoothing differ? What are their respective advantages?
	c. Why is the WL/WP slider restricted to values between 0.001 and 0.999? What would happen for values of 0 and 1, respectively?
	d. How are boundaries handled by your implementation of the mesh Laplacian? Are there any undesirable effects when smoothing meshes with boundary?
	e. Why does spectral smoothing not work with the normalized Laplacian?
	f. What happens if spectral smoothing is applied with a very low number of eigenfunctions (k=1,2,3)? Why does this happen?
